...
def read_examples(input_path, tokenizer, op_list, const_list, log_file, mode):
    ...
    for entry in input_data:
        example = finqa_utils.read_mathqa_entry(entry, tokenizer)
        if mode == 'train' and len(example.all_positive) == 0:
            continue
...
def retrieve_evaluate(all_logits, all_filename_ids, all_inds, output_prediction_file, ori_file, topn):
    ...
    for d in data_all:
        ctxs = []
        ctxs.extend([{
            'text': d['pre_text'][int(x['ind'].split('_')[-1])],
            'idx': x['ind'],
            'score': x['score']
        } for x in d['text_retrieved_all']])
        ctxs.extend([{
            'text': table_row_to_text(d['table'][0], d['table'][int(x['ind'].split('_')[-1])]),
            'idx': x['ind'],
            'score': x['score']
        } for x in d['table_retrieved_all']])
        result.append({
            "question": d['qa']['question'],
            'id': d['id'],
            'answers': [x for _, x in d['qa']['gold_inds'].items()],
            'ctxs': ctxs
        })
    ...
    if all_count != 0:
        res_3 = all_recall_3 / all_count
        res = all_recall / all_count
    else:
        res_3 = 0
        res = 0
