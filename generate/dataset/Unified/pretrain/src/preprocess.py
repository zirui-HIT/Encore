...
def variable_replace(derivation: str, variables: List[str]) -> str:
    for i in range(len(variables) - 1, -1, -1):
        derivation = derivation.replace(f"x{i}", variables[i])
    return derivation
...
def generate_table_data(table) -> List[Tuple[str, str]]:
    ...
    def table_column_calculation(table: Tabular) -> List[Tuple[str, str]]:
        ...
        for i in range(1, table.col_number):
            ...
            # sum
            if len(all_number) > 1:
                derivations: List[str] = ["add ( x0 , x1 )"]
                for i in range(len(all_number) - 2):
                    derivations.append(f"add ( #{i} , x{i + 2} )")
            ...
            if len(all_number) > 1:
                ...
                for i in range(len(all_number) - 2):
                    derivations.append(f"add ( #{i} , x{i + 2} )")
                derivations.append(
                    f"divide ( #{len(derivations) - 1} , {len(all_number)} )")
                derivation = ' | '.join(derivations)
                variables = [table.locate(i, x[0]) for x in all_number]
    ...
    result = result + table_expand_location(table)[:len(result) // 2]
    return result


if __name__ == '__main__':
    from generate.bart.dataset import Tabular
    from generate.utils import duplicate_remove, calc_derivation
