...
MONTHS = {v.lower(): i+1 for i, v in enumerate(["January", "February", "March", "April",
                                                "May", "June", "July", "August", "September", "October", "November", "December"])}
FLAG_NER = "tp@ckl"
FLAG_SENTENCE = "tp#ckl"
NUM_NER_TYPES = ['ENT2NUM', 'NUMBER', 'PERCENT', 'MONEY',
                 'TIME', 'DATE', 'DURATION', 'ORDINAL', 'YARD']
...
def retrieve_dates(passage_text):
    ...
    for i, token in enumerate(tokens):
        ...
        if month is not None:
            ...
            if day is None:
                day = 1
            dates.append((sidx, eidx, year, month, day,
                         ' '.join(tokens[sidx:eidx+1])))
            if sidx2 is not None:  # and sidx2 > dates[-1][1]:
                print('match!!!!!!', ' '.join(tokens[sidx2:eidx+1]))
                dates.append((sidx2, sidx2, year, month,
                             int(get_day(tokens[sidx2]))))
        else:
...
#            res_tokens.append(date_tokens[-1]+FLAG_DATE)
                        ...
                        res_tokens.append(
                            normalize_day_month(str(date[3]))+FLAG_DATE)
                        res_tokens.append(
                            normalize_day_month(str(date[4]))+FLAG_DATE)
                        didx += 1
                    continue
...
def drop_tokenize(text, tokenizer, is_answer=False):
    ...
    for i in range(len(tokens)):
        ...
        if pos > 0:
            ...
            tokens[i] = tokens[i][:pos]+tokens[i][pos2 +
                                                  len(FLAG_NER):]  # remove entity flag
            # and i == sentences_entites[-1][-1][0]+sentences_entites[-1][-1][-1]:
            if ner_type == pre_ner_type and len(sentences_entites[-1]) > 0:
                sentences_entites[-1][-1] = (sentences_entites[-1]
                                             [-1][0], ner_type, sentences_entites[-1][-1][-1]+1)
            else:
...
#    old_index, j, new_exclude_indices = 0, 0, []
    ...
    for i, c in enumerate(new_text):
        ...
        # or c in ["-", "–", "~"]:
        if is_whitespace(c) or i == len(new_text)-1:
            if sid < len(sentences_entites) and tid >= 0 and tid < len(sentences_entites[sid]):
                if tkid == sentences_entites[sid][tid][0]:
                    if sentences_entites[sid][tid][-1] > 1:
                        ...
                        sentences_entites[sid][tid] = (
                            len(tokens)-1, sentences_entites[sid][tid][1], 1)
                        tid = tid + \
                            1 if tid < len(sentences_entites[sid])-1 else -2
                elif tkid == sentences_entites[sid][tid][0] + sentences_entites[sid][tid][-1] - 1:
                    sentences_entites[sid][tid] = (
                        cid, sentences_entites[sid][tid][1], len(tokens)-cid)
    ...
    for i, token in enumerate(tokens):
        ...
        if sid < len(new_sentences_entites) and len(new_sentences_entites[sid]) > 0 and tid >= 0 and i == new_sentences_entites[sid][tid][-1] + new_sentences_entites[sid][tid][0]-1:
            new_sentences_entites[sid][tid] = (
                cid, new_sentences_entites[sid][tid][1], len(split_tokens)-cid)
            ...
            new_sentences_num[sid][ntid] = (
                cid, new_sentences_num[sid][ntid][1], len(split_tokens)-cid)
        ...
        if sid < len(sentence_ids) and i == sentence_ids[sid]:
            ...
            for ent_id in ent_ids:
                for num_id in num_ids:
                    gedges.append(
                        [ent_id, num_id, split_tokens[ent_id], split_tokens[num_id], edge_type])
            ent_ids, num_ids = [], []
        ...
        if gnodes_type[i] in NUM_NER_TYPES:
            ...
            for j in range(i+1, len(gnodes)):
                if gnodes_type[j] == gnodes_type[i]:
                    gedges.append([i, j, split_tokens[gnodes[i]],
                                  split_tokens[gnodes[j]], gedge_type])
...
class DropReader(object):
        ...
        for passage_id, passage_info in tqdm(dataset.items()):
            ...
            for question_answer in passage_info["qa_pairs"]:
                ...
                if instance is not None:
                    ...
                    skip_count += 1
        print(
            f"Skipped {skip_count} questions, kept {len(instances)} questions.")
        return instances
    ...
    def text_to_instance(self,
        ...
        question_number_indices, question_number_len, numbers_in_question = clipped_passage_num(
            question_number_indices, question_number_len, numbers_in_question, len(
                question_tokens)
        )

        qgnodes, qgnodes_len, qgnodes_type, qgedges = clipped_graph(
            qgnodes, qgnodes_len, qgnodes_type, qgedges, len(question_tokens))

        qp_tokens = ["<s>"] + question_tokens + ["</s>"] + passage_tokens
        qp_wordpiece_mask = [1] + question_wordpiece_mask + \
            [1] + passage_wordpiece_mask
        ...
        if len(qp_tokens) > self.max_pieces - 1:
            ...
            gnodes, gnodes_len, gnodes_type, gedges = clipped_graph(
                gnodes, gnodes_len, gnodes_type, gedges, plen)
        ...
        if answer_annotations:
            ...
            answer_texts = [" ".join(whitespace_tokenize(answer_text))
        ...
        for answer_text in answer_texts:
            answer_tokens, _, _, _, _, _, _, _, _, _, _ = drop_tokenize(
        ...
        if self.instance_format == "drop":
            ...
            if all_number_order is None:
                ...
                passage_number_order = all_number_order[:len(
                    numbers_in_passage)]
                question_number_order = all_number_order[len(
                    numbers_in_passage):]
            ...
            if answer_type in ["number"]:
                ...
                valid_counts = self.find_valid_counts(
            ...
            if specific_answer_type == MULTI_SPAN and (len(valid_passage_spans) > 0 or len(valid_question_spans) > 0):
                ...
                for tokenized_answer_text in tokenized_answer_texts:
                    spans = self.find_valid_spans(
                        qp_tokens, [tokenized_answer_text])
                    if len(spans) == 0:
                ...
                if (flexibility_count < self.flexibility_threshold):
                    ...
                    for key, spans in spans_dict.items():
                        ...
                        for i in range(1, len(spans) + 1):
                            all_combinations += list(
                                itertools.combinations(spans, i))
                    ...
                    for packed_gold_spans in packed_gold_spans_list:
                        gold_spans = [
                            s for sublist in packed_gold_spans for s in sublist]
                ...
                multi_span = [is_bio_mask, answer_as_text_to_disjoint_bios,
    ...
    def find_valid_spans(passage_tokens: List[str], answer_texts: List[str]) -> List[Tuple[int, int]]:
        normalized_tokens = [token.strip(
            USTRIPPED_CHARACTERS).lower() for token in passage_tokens]
        ...
        spans = []
        for answer_text in answer_texts:
            answer_tokens = [token.strip(USTRIPPED_CHARACTERS).lower()
...
def sample_uid() -> str:
    random.sample("abcdefghijklmnopqrstuvwxyz0123456789", 16)


if __name__ == '__main__':
    random.seed(42)
    for mode in ['test']:
        data_path = f'./dataset/DROP/data/{mode}.json'
        output_path = f'dataset/DROP/origin/{mode}.json'

        with open(data_path, 'r', encoding='utf-8') as f:
            origin_data: Dict[str, Any] = json.load(f)

        tokenizer = BartTokenizer.from_pretrained("facebook/bart-large")
        reader = DropReader(tokenizer)
        instances = reader._read(data_path)

        i = 0
        result: List[Dict[str, Any]] = []
        for k, d in origin_data.items():
            result.append({})
            result[-1]['table'] = {
                "uid": sample_uid(),
                "table": []
            ...
            for pair in d['qa_pairs']:
                x = instances[i]
                i += 1

                result[-1]['questions'].append({
                    "uid": pair['query_id'],
                    "order": len(result[-1]['questions']) + 1,
                ...
                if 'answer' in pair:
                    if pair['answer']['number']:
                        if x['answer_info']['counts']:
                            result[-1]['questions'][-1]['answer_type'] = 'count'
                            result[-1]['questions'][-1]['answer'] = pair['answer']['number']
                            continue

                        result[-1]['questions'][-1]['answer_type'] = 'number'
                        result[-1]['questions'][-1]['answer'] = pair['answer']['number']
                        result[-1]['questions'][-1]['expressions'] = []
                        if x['answer_info']['signs_for_add_sub_expressions']:
                            result[-1]['questions'][-1]['expressions'] = x['answer_info']['signs_for_add_sub_expressions']
                            result[-1]['questions'][-1]['variables'] = x['original_numbers']
                            for idx, value in enumerate(x['original_numbers']):
                                if str(value) == result[-1]['questions'][-1]['answer']:
                                    result[-1]['questions'][-1]['expressions'].append(
                                        [1 if t == idx else 0 for t in range(len(x['original_numbers']))])
                    elif pair['answer']['spans']:
                        result[-1]['questions'][-1]['answer_type'] = 'spans'
                        result[-1]['questions'][-1]['answer'] = pair['answer']['spans']
                    else:
                        result[-1]['questions'][-1]['answer_type'] = 'date'
                        date: List[str] = [
                            x for x in pair['answer']['date'].values() if x]
                        result[-1]['questions'][-1]['answer'] = "##".join(date)
